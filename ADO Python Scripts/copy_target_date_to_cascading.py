#!/usr/bin/env python3
"""
Copy Custom.Target_Date to Custom.CascadingDate
...
def query_work_items():
    """Query for all work items with Custom.Target_Date populated"""
    print("Querying for work items with Custom.Target_Date...")
    
    wiql = """
    SELECT [System.Id]
    FROM WorkItems
    WHERE [System.TeamProject] = 'eShare'
      AND [Custom.Target_Date] <> ''
    ORDER BY [System.Id]
    """
    ...
    print(f"Found {len(work_item_ids)} work items with Custom.Target_Date")
    ...
    for i in range(0, len(ids), BATCH_SIZE):
        ...
        items = wit_client.get_work_items(
            ...
            fields=['System.WorkItemType', 'System.Title', 
                   'Custom.Target_Date', 'Custom.CascadingDate']
        )
...
def prepare_updates(work_items):
    ...
    for wi in work_items:
        target_date = wi.fields.get('Custom.Target_Date')
        ...
        # Skip if Target_Date is empty string
        if target_date.strip() == '':
            stats['empty_target_date'] += 1
            continue
        
        # Check if update needed
        if cascading_date == target_date:
    ...
    print(f"  Empty target date: {stats['empty_target_date']}")
...
def main():
    print("=" * 80)
    print("COPY: Custom.Target_Date → Custom.CascadingDate")
    print("=" * 80)
    print()
    ...
    try:
        ...
        if failed:
            ...
            invalid_picklist = [f for f in failed if 'is not a valid list value' in f['error'].lower() 
                                or 'picklist' in f['error'].lower()]
            if invalid_picklist:
                print(f"\n⚠️  {len(invalid_picklist)} failures appear to be invalid picklist values.")
                print("You may need to add these values to the picklist first:")
