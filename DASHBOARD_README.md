...
| Version | Date | Notes |
|---------|------|-------|
| v85 | 12/15/2025 | **Generic Priority Filter Component:** Created shared Priority filter in part2.html for Releases, Roadmap, and Customers dashboards. Shows P1, P2, P3, P4 with item counts (sorted numerically), "(No Priority)" option last with count. Includes search box at top, 200px wide dropdown. Cross-filter aware: dropdown options update based on other active filters. **Generic State Filter Component:** Created shared State filter in part2.html for same dashboards. States sorted semantically (New → Triaged → To Do → In Progress → Ready For Review → Done → Closed → Removed). Shows "(No State)" option last with count. 200px wide dropdown with search box and item counts. Full cross-filter support. **Releases Dashboard:** Added Priority filter after Customer (before Type), refactored State filter to use generic component with item counts and cross-filter support. **Roadmap Dashboard:** Refactored to use generic Priority and State components, removed dataset.populated caching. **Customers Dashboard:** Refactored to use generic Priority and State components, cross-filter aware. **Priority functions:** `computePriorityInfo()`, `buildPriorityFilterDropdown()`, `filterGenericPriorityOptions()`, `handleGenericPriorityChange()`, `selectAllGenericPriority()`, `clearGenericPriority()`, `updateGenericPriorityDisplay()`, `syncGenericPriorityFilter()`. **State functions:** `computeStateInfo()`, `buildStateFilterDropdown()`, `filterGenericStateOptions()`, `handleGenericStateChange()`, `selectAllGenericState()`, `clearGenericState()`, `updateGenericStateDisplay()`, `syncGenericStateFilter()`. |
