...
| Version | Date | Notes |
|---------|------|-------|
| v74 | 12/9/2025 | **Roadmap view:** Added Release Version filter dropdown (between Iteration and Tag filters). Default filter is "(No Release)" to show Features with no release version assigned. Dual-mode filter behavior: inclusion mode for manual selections, exclusion mode after "Select All". Clear All preserves both "(No Release)" and "Candidate" defaults. Updated Info popup to document default filters. **Bug fix:** Team + Iteration filters now correctly require slices to match BOTH filters (previously evaluated independently). |
