...
| Version | Date | Notes |
|---------|------|-------|
| v86 | 12/15/2025 | **Generic State Filter Component:** Created shared State filter in part2.html for Releases, Roadmap, and Customers dashboards. States sorted semantically (New → Triaged → To Do → In Progress → Ready For Review → Done → Closed → Removed). Shows "(No State)" option last with count. Includes search box at top, 200px wide dropdown with item counts. Full cross-filter support. **Releases Dashboard:** Refactored State filter to use generic component with item counts and cross-filter support. **Roadmap Dashboard:** Refactored to use generic State component, removed dataset.populated caching. **Customers Dashboard:** Refactored to use generic State component, cross-filter aware. **Functions added:** `computeStateInfo()`, `buildStateFilterDropdown()`, `filterGenericStateOptions()`, `handleGenericStateChange()`, `selectAllGenericState()`, `clearGenericState()`, `updateGenericStateDisplay()`, `syncGenericStateFilter()`. |
| v85 | 12/15/2025 | **Generic Priority Filter Component:** Created shared Priority filter in part2.html for Releases, Roadmap, and Customers dashboards. Shows P1, P2, P3, P4 with item counts (sorted numerically), "(No Priority)" option last with count. Includes search box at top, 200px wide dropdown. **Releases Dashboard:** Added Priority filter after Customer filter (before Type). Cross-filter aware: dropdown options update based on other active filters. **Roadmap Dashboard:** Refactored to use generic Priority component, updated filter logic to use P1/P2/P3/P4 format. **Customers Dashboard:** Refactored to use generic Priority component. **Functions added:** `computePriorityInfo()`, `buildPriorityFilterDropdown()`, `filterGenericPriorityOptions()`, `handleGenericPriorityChange()`, `selectAllGenericPriority()`, `clearGenericPriority()`, `updateGenericPriorityDisplay()`, `syncGenericPriorityFilter()`. |
