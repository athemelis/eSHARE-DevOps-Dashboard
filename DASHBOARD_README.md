...
| v96 | 12/17/2025 | **Tasks Dashboard - Work Log Summary & Gantt Timeline:** Added Work Log Summary section at top of Tasks dashboard showing work logged in past 7 days. **Stats row:** Total days logged, engineer count, task count, entry count. **BY WORK ITEM section:** Aggregates work by meaningful parent (Feature, Bug, Issue, Delivery Slice) with orphan warnings for tasks without parents. **TIMELINE section:** Gantt chart showing task bars per engineer grouped by team. Dynamic weekend handling - shows only weekdays unless weekend work is logged. Activity types color-coded (Coding=blue, Testing=green, Design=purple, Other=gray). Clickable bars open task in ADO. **Data validation:** Yellow warning banner for entries where effort exceeds date range (e.g., 7 days logged for a single day - likely hours entered as days). Affected bars shown with dashed yellow border and ⚠ indicator. **Timezone fix:** Work log dates now parse as local dates to avoid UTC offset issues (same fix as v92 for Target Date). **Releases Dashboard - Collapsible Tables:** All 4 tables (Features, Issues, Customer Bugs, Internal Bugs) now collapsible. Issues table collapsed by default, others expanded. Collapse state persisted via localStorage. |
...
| v91 | 12/16/2025 | **Bugs Dashboard Redesign:** Simplified layout with Bug Trend by Week chart (fixed past quarter timeframe), new Bugs table with columns: ID, Title, State, Customers, Priority, Aging, Tags, Bug Owner, Release, Target Date. Archive Views section (collapsible, collapsed by default) containing categorization and effort charts. Removed Date Range filter from sticky header. **Generic Relationship Pills:** Created shared `countRelationshipsForWorkItem()` and `buildRelationshipPills()` functions in part2.html. Shows colored pills under title: "has parent" (orange), "X children" (cyan), "X related" (purple), "no links" (gray). **All Tables Now Show Relationship Pills:** Updated default cell renderer in `genericTableDefaultRenderCell()` to include relationship pills in title column. Applied to Releases (Features, Issues, Customer Bugs, Internal Bugs), Roadmap, Bugs, and Customers tables. Row click opens Work Item Details modal with full relationship display. |
| v90 | 12/16/2025 | **Generic Tag Filter Component:** Created shared Tag filter in part2.html for Releases, Roadmap, and Customers dashboards. Includes search box at top, Match toggle (OR/AND logic), Select All/Clear buttons, and dual-mode filtering (inclusion/exclusion). Added after State filter in all three dashboards. Roadmap Tag filter refactored to use generic component. **Customers Dashboard - Enhancement Request Prioritization Summary:** New collapsible summary table (always visible by default) showing CS tag distribution for Enhancement Requests. Three columns: "CS: High Value", "CS: Low Value", "CS: Strategic" with issue counts and percentages. Footer shows count of Enhancement Requests without CS tags (clickable to filter). Collapse state persisted via localStorage. **Customers Dashboard - Pipe-Separated ID Search:** Added `supportPipeSeparated: true` option to search filter, enabling filtering by multiple IDs (e.g., `123\|456\|789`). Used by "Issues without CS tags" click-to-filter feature. |
| v89 | 12/16/2025 | **Consistent Filter Display Behavior:** Created shared `formatFilterDisplay()` helper function ensuring all filter dropdowns show: 0 items = "All X", 1 item = actual value, 2 items = comma-separated (e.g., "Done, Closed"), 3+ items = "N items" (e.g., "3 states"). Applied to all generic filters (Release, Customer, Priority, State, Team) and dashboard-specific filters across Releases, Roadmap, Customers, and other views. **Filter Display Bug Fixes:** Fixed Releases dashboard State and Customer filters not showing actual value when 1 item selected (display ID mismatches). Added missing `updateGenericStateDisplay()` call in state filter change handler. **Releases Dashboard State Persistence:** Fixed bug where filter state was lost when navigating away from Releases dashboard and returning. Removed erroneous `selectedReleases = []` clearing in `switchView()` function. Filter state now persists in memory during navigation (consistent with Roadmap and Customers dashboards). |
